...
export async function chat(req, res) {
  ...
  const result = await askGrounded({ userId: req.user._id, question, documentId, history, redactPII: req.user.preferences?.redactPIIForAI !== false });
  conversation.messages.push({ role:'user', content: question }, { role:'assistant', content: result.answer, citations: result.citations.map(c => ({ label:c.label, sourceId:c.sourceId, chunkId:c.chunkId })) });
  if (conversation.messages.length > 60) conversation.messages = conversation.messages.slice(-60);
  await conversation.save();
  await audit({ userId:req.user._id, action:'AI_CHAT', resourceType: documentId ? 'document' : 'knowledge_base', resourceId: documentId, metadata:{ grounded: result.citations.length > 0 }, ip:req.ip });
  return ok(res, { conversationId: conversation._id, ...result }, 'Answer generated from authorized retrieved context');
}
